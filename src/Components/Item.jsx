      ...
      <div className='flex justify-between'>
        {!added && (
          <button
            onClick={() => {
              dispatch(
                addItem({ id, title, price, desc, rating, quantity: 1 })
              );
              handleAdd();
            }}
        ...
        {added && (
          <button className=' flex border border-green-500 text-green-500 text-sm focus:outline-none hover:bg-green-500 hover:text-white py-1.5 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px] md-mx:text-xs md-mx:py-1 md-mx:px-2'>
            <CheckIcon className='font-extrabold w-5 h-5' /> Added
          </button>
        )}
      </div>
    </div>
